...
const saveGeolocalizacion = async(req, res)=> {
  ...
  const getSearchGeo = async (req, res) => {
    //console.log(req.body);
    const {razon_social} = req.body;
    //console.log(razon_social);
    try {
      const datos = await GeoModel.find({razon_social});
      if(!datos) return res.status(401).Json('No existen los datos ');
      else{
        return res.status(200).json(datos);
      }
    } catch (error) {
  ...
  module.exports = {
    pruebas,  saveGeolocalizacion, getLocalizacion, getItemGeo, getSearchGeo
}