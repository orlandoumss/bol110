...
function saveUser(req, res){
    ...
    if(params){
            ...
            user.nomb_user = params.username;
            user.rol_user = params.rol;
            ...
            bcrypt.hash(params.password, null, null, function(err, hash){
                ...
                user.save((err, userStored) => {
                    if(err){
                        ...
                        if(!userStored){
                            res.status(404).json({message: 'no se ha registrado el usuario'});
                        }else{
...
//     // res.status(200).send({mrssage: 'metodo del login'});
...
const login = async (req, res) => {
    ...
    bcrypt.compare(password, user.password, (err, result) => {
        ...
        if(result) {
            ...
            return res.status(401).json({auth: result, token: null});
        }
    });
}
